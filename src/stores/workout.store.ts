...
export const useWorkoutStore = create<ActiveWorkoutState & WorkoutActions>((set) => ({
  ...
  startWorkout: (sessionId, programDayId, _programId) =>
    set({
  ...
  addOptimisticSet: (exerciseId, set_) =>
    set((state) => ({
      logged_sets: {
        ...state.logged_sets,
        [exerciseId]: [
          ...(state.logged_sets[exerciseId] || []),
          { ...set_, isPending: true } as OptimisticWorkoutSet,
        ],
      },
    })),

  // Replace optimistic set with confirmed server response
  confirmSet: (exerciseId, tempId, confirmedSet) =>
    set((state) => ({
      logged_sets: {
        ...state.logged_sets,
  ...
  startRestTimer: (totalSeconds, exerciseName) =>
    set({
      rest_timer: {
        is_running: true,
