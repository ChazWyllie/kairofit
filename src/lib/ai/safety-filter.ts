...
const BLOCKED_PATTERNS: Array<[RegExp, SafetyCategory, string]> = [
  ...
  [
    /do i have (diabetes|cancer|injury|torn|herniated|arthritis)/i,
    'medical_diagnosis',
  ...
  // Extreme restriction
  [
    /\b([0-9]{1,3})\s*calories\s*(per day|daily|a day)/i,
    'extreme_restriction',
    'Potential extreme calorie restriction',
  ],
  ...
  [
    /thinspo|fitspo.*starvation|pro.?ana/i,
    'eating_disorder',
    'Eating disorder community reference',
  ],

  // Injury override attempts
  [
    /ignore (my|the) (injury|pain|shoulder|knee|back)/i,
    'injury_override',
    'Injury override attempt',
  ],
  ...
  [
    /\b(trenbolone|testosterone|sarms|winstrol|anavar)\s+(cycle|dose|protocol)/i,
    'ped_request',
    'PED protocol request',
  ],
...
export async function checkInputSafety(input: string): Promise<SafetyResult> {
  ...
  const fitnessTerms =
    /\b(workout|exercise|training|gym|muscle|strength|cardio|sets|reps|lift|squat|deadlift|bench|program|split|recovery|protein|nutrition|weight|body|fitness|run|swim|cycle)\b/i
...
export const SAFETY_MESSAGES: Record<Exclude<SafetyCategory, 'safe'>, string> = {
  medical_diagnosis:
    'I can help with training and programming, but medical questions need a qualified healthcare provider.',
  extreme_restriction:
    'That level of restriction is outside what I can safely recommend. For guidance on nutrition targets, I can suggest evidence-based approaches for your goal.',
  eating_disorder:
    'I am not able to help with that. If you are struggling with your relationship with food, please reach out to a healthcare provider or call the NEDA Helpline.',
  injury_override:
    'Your injury flags are there to protect you. I cannot recommend exercises that contradict your injury profile - but I can help find effective alternatives that work around it.',
  ped_request:
    'I only cover evidence-based natural training. Performance-enhancing drug protocols are outside my scope.',
  out_of_scope:
    'I can only help with fitness, training, and nutrition questions. What would you like to work on today?',
}
